...
Always **pin the version** (here `0.4.1`, matching `Cargo.toml` / `package.json`).
...
import init, { JSONstat, init_panic_hook }
  from 'https://cdn.jsdelivr.net/npm/jsonstat-wasm@0.4.1/jsonstat_wasm.js';
...
```js
import init, { JSONstat } from 'https://esm.sh/jsonstat-wasm@0.4.1/glue';

await init('https://esm.sh/jsonstat-wasm@0.4.1/jsonstat_wasm_bg.wasm');
const ds = new JSONstat(jsonStr);
```
